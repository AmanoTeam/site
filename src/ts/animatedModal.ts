...
var animatedModal = function (target, options) {
  ...
  target.addEventListener("click", function (event) {
    ...
    document
      ...
      .forEach((element) => (element.style.overflow = "hidden"));
    if (href == idConc) {
      ...
      if (id.classList.contains(settings.modalTarget + "-on")) {
        ...
        [
          ...
          id.addEventListener(eventName, afterOpen, { once: true }),
        );
      }
    }
  });
  ...
  closeBt.addEventListener("click", function (event) {
    ...
    document
      .querySelectorAll("body, html")
      .forEach((element) => (element.style.overflow = "auto"));
    ...
    if (id.classList.contains(settings.modalTarget + "-off")) {
      ...
      [
        ...
        id.addEventListener(eventName, afterClose, { once: true }),
      );
    }
  });
